...
# MAC_DB="$REPO_DIR/network/mac_addresses"
...
REPO_DIR="/home/debix/rover-setup/network"
...
CUSTOM_NAME_PATH="$REPO_DIR/custom_hostname"
...
if [ -z "$(cat $REPO_DIR/mac_db | grep "$DB_STRING")" ]; then
	debug "adding '$DB_STRING' to network/mac_db"
	echo $DB_STRING >> $REPO_DIR/mac_db
else
