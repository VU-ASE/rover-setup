...
To install OpenCV and GoCV, you can normally just run the install steps on the [GoCV github](https://github.com/hybridgroup/gocv). Though, because you have to build from source on the debix, this takes a while (about 4 hours) and so is not feasible to do with every ansible update.

Hence, we have modified the GoCV Makefile to extract the build files. These build files can then be `scp`ed onto the debix, after which you just have to run the `sudo_install` step from the original Makefile like so:

```bash
# cd $(TMP_DIR)opencv/opencv-$(OPENCV_VERSION)/build
cd prebuilt-deps/gocv-build/opencv-4.10.0/build
sudo make install
sudo ldconfig
```

This saves a lot of time. You can use this setup for never versions of GoCV as well using the Makefile below (notice how the `TEMP_DIR` and `build` target have changed).
